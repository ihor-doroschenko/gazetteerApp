import { eventHandlersSubtable } from './eventHandlersSubtable';
...
it('validate if a row receives a signal color if moused', () => {
  ...
  const expectedValue = eventHandlersSubtable(id, gazName, mouseOverElement, subTableStylesData);
  expect(JSON.stringify(expectedValue)).toBe(
    JSON.stringify({
      onMouseOver: () => {
        dispatch(mouseOverMarker({ gazName: 'gnd', id: '12345' }));
      },
      onMouseLeave: () => {
